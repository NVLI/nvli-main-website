...
vjs.CoreObject.extend = function(c) {
    var d, a;
    c = c || {};
...
vjs.on = function(c, b, a) {
    ...
    if (d.handlers[b].length == 1) {
        if (document.addEventListener) {
            c.addEventListener(b, d.dispatcher, false)
        } else {
            if (document.attachEvent) {
                c.attachEvent("on" + b, d.dispatcher)
            }
        }
    }
};
vjs.off = function(e, d, c) {
    if (!vjs.hasData(e)) {
        return
    }
    var f = vjs.getData(e);
...
vjs.cleanUpEvents = function(b, a) {
    var c = vjs.getData(b);
    if (c.handlers[a].length === 0) {
        delete c.handlers[a];
        if (document.removeEventListener) {
            b.removeEventListener(a, c.dispatcher, false)
        } else {
            if (document.detachEvent) {
                b.detachEvent("on" + a, c.dispatcher)
            }
        }
    }
...
vjs.trigger = function(d, c) {
    var a = (vjs.hasData(d)) ? vjs.getData(d) : {};
    ...
    c = vjs.fixEvent(c);
    if (a.dispatcher) {
        a.dispatcher.call(d, c)
    }
    if (b && !c.isPropagationStopped() && c.bubbles !== false) {
        vjs.trigger(b, c)
    } else {
        if (!b && !c.isDefaultPrevented()) {
            var e = vjs.getData(c.target);
            if (c.target[c.type]) {
                e.disabled = true;
                if (typeof c.target[c.type] === "function") {
                    c.target[c.type]()
                }
                e.disabled = false
            }
...
vjs.obj = {};
vjs.obj.create = Object.create || function(b) {
    function a() {}
    a.prototype = b;
    return new a()
};
vjs.obj.each = function(d, c, b) {
    for (var a in d) {
        if (hasOwnProp.call(d, a)) {
            c.call(b || this, a, d[a])
        }
    }
};
vjs.obj.merge = function(c, b) {
    if (!b) {
        return c
    }
    for (var a in b) {
        if (hasOwnProp.call(b, a)) {
            c[a] = b[a]
        }
    }
    return c
};
vjs.obj.deepMerge = function(e, d) {
    var a, c, b;
    e = vjs.obj.copy(e);
    for (a in d) {
        if (hasOwnProp.call(d, a)) {
            c = e[a];
            b = d[a];
            if (vjs.obj.isPlain(c) && vjs.obj.isPlain(b)) {
                e[a] = vjs.obj.deepMerge(c, b)
            } else {
                e[a] = d[a]
            }
        }
...
vjs.obj.isPlain = function(a) {
    return !!a && typeof a === "object" && a.toString() === "[object Object]" && a.constructor === Object
};
vjs.bind = function(c, d, b) {
    if (!d.guid) {
        d.guid = vjs.guid++
    }
    var a = function() {
...
vjs.ANDROID_VERSION = (function() {
    var b = vjs.USER_AGENT.match(/Android (\d+)(?:\.(\d+))?(?:\.(\d+))*/i),
        a, c;
    if (!b) {
        return null
    }
...
vjs.getAttributeValues = function(a) {
    var g, b, d, f, c;
    g = {};
    ...
    if (a && a.attributes && a.attributes.length > 0) {
        ...
        for (var e = d.length - 1; e >= 0; e--) {
            f = d[e].name;
            c = d[e].value;
            if (typeof a[f] === "boolean" || b.indexOf("," + f + ",") !== -1) {
                c = (c !== null) ? true : false
            }
            g[f] = c
        }
...
vjs.formatTime = function(g, b) {
    b = b || g;
    var e = Math.floor(g % 60),
        a = Math.floor(g / 60 % 60),
        d = Math.floor(g / 3600),
...
vjs.createTimeRange = function(b, a) {
    return {
        length: 1,
        start: function() {
            return b
...
vjs.setLocalStorage = function(b, c) {
    try {
        var a = window.localStorage || false;
        if (!a) {
            return
        }
        a[b] = c
...
vjs.findPosition = function(c) {
    var g, b, h, j, e, d, f, a, k;
    if (c.getBoundingClientRect && c.parentNode) {
...
vjs.Component.prototype.options = function(a) {
    ...
    return this.options_ = vjs.obj.deepMerge(this.options_, a)
};
vjs.Component.prototype.el_;
vjs.Component.prototype.createEl = function(b, a) {
    return vjs.createEl(b, a)
};
...
vjs.Component.prototype.addChild = function(f, d) {
    var c, e, a, b;
    if (typeof f === "string") {
        a = f;
        d = d || {};
...
vjs.Component.prototype.removeChild = function(b) {
    if (typeof b === "string") {
        b = this.getChild(b)
    }
    if (!b || !this.children_) {
...
vjs.Component.prototype.initChildren = function() {
    ...
    if (b && b.children) {
        var a = this;
        vjs.obj.each(b.children, function(d, e) {
            if (e === false) {
                return
            }
            var c = function() {
...
vjs.Component.prototype.on = function(b, a) {
    vjs.on(this.el_, b, vjs.bind(this, a));
    return this
};
vjs.Component.prototype.off = function(b, a) {
    vjs.off(this.el_, b, a);
    return this
};
vjs.Component.prototype.one = function(b, a) {
    vjs.one(this.el_, b, vjs.bind(this, a));
    return this
};
vjs.Component.prototype.trigger = function(a, b) {
    vjs.trigger(this.el_, a, b);
    return this
};
...
vjs.Component.prototype.dimensions = function(b, a) {
    return this.width(b, true).height(a)
};
vjs.Component.prototype.dimension = function(e, a, d) {
    if (a !== undefined) {
    ...
    var b = c.indexOf("px");
    if (b !== -1) {
        return parseInt(c.slice(0, b), 10)
    } else {
        return parseInt(this.el_["offset" + vjs.capitalize(e)], 10)
    }
};
...
vjs.Button.prototype.createEl = function(b, a) {
    a = vjs.obj.merge({
...
vjs.Slider.prototype.createEl = function(b, a) {
    a = a || {};
...
vjs.Slider.prototype.calculateDistance = function(b) {
    var d, g, m, l, a, j, h, f, e;
    d = this.el_;
    ...
    if (this.options_.vertical) {
        ...
        m = g.left;
        if (b.changedTouches) {
            f = b.changedTouches[0].pageX
        } else {
            f = b.pageX
        }
        if (h) {
            var c = h.el().offsetWidth;
            m = m + (c / 2);
            a = a - c
        }
...
vjs.SliderHandle.prototype.createEl = function(b, a) {
    a = a || {};
...
vjs.Menu.prototype.createEl = function() {
    var b = this.options().contentElType || "ul";
    this.contentEl_ = vjs.createEl(b, {
...
vjs.MenuItem = vjs.Button.extend({
    init: function(b, a) {
        vjs.Button.call(this, b, a);
        this.selected(a.selected)
    }
});
vjs.MenuItem.prototype.createEl = function(b, a) {
...
vjs.MenuButton.prototype.createMenu = function() {
    var b = new vjs.Menu(this.player_);
    if (this.options().title) {
        b.el().appendChild(vjs.createEl("li", {
...
vjs.Player = vjs.Component.extend({
    init: function(a, b, c) {
        ...
        if (b.plugins) {
            vjs.obj.each(b.plugins, function(d, e) {
                this[d](e)
            }, this)
        }
...
vjs.Player.prototype.getTagSettings = function(a) {
    ...
    if (a.hasChildNodes()) {
        ...
        for (e = 0, c = f.length; e < c; e++) {
            ...
            if (b === "source") {
                d.sources.push(vjs.getAttributeValues(g))
            } else {
                if (b === "track") {
                    d.tracks.push(vjs.getAttributeValues(g))
                }
            }
        }
    }
...
vjs.Player.prototype.loadTech = function(d, b) {
    if (this.tech) {
        this.unloadTech()
    } else {
        if (d !== "Html5" && this.tag) {
...
vjs.Player.prototype.techCall = function(c, a) {
...
vjs.Player.prototype.techGet = function(b) {
    if (this.tech && this.tech.isReady_) {
        try {
            return this.tech[b]()
        } catch (a) {
            if (this.tech[b] === undefined) {
                vjs.log("Video.js: " + b + " method not defined for " + this.techName + " playback technology.", a)
            } else {
                if (a.name == "TypeError") {
                    vjs.log("Video.js: " + b + " unavailable on " + this.techName + " playback technology element.", a);
...
vjs.Player.prototype.currentTime = function(a) {
    if (a !== undefined) {
        this.cache_.lastSetCurrentTime = a;
...
vjs.Player.prototype.duration = function(a) {
    //console.log(a);
    if (a !== undefined) {
        this.cache_.duration = parseFloat(a);
        return this
    }
...
vjs.Player.prototype.buffered = function() {
    var c = this.techGet("buffered"),
        d = 0,
        a = c.length - 1,
        b = this.cache_.bufferEnd = this.cache_.bufferEnd || 0;
    if (c && a >= 0 && c.end(a) !== b) {
        b = c.end(a);
...
vjs.Player.prototype.requestFullScreen = function() {
    ...
    if (a) {
        vjs.on(document, a.eventName, vjs.bind(this, function(b) {
            this.isFullScreen = document[a.isFullScreen];
            if (this.isFullScreen === false) {
                vjs.off(document, a.eventName, arguments.callee)
            }
...
vjs.Player.prototype.selectSource = function(g) {
    for (var h = 0, f = this.options_.techOrder; h < f.length; h++) {
        var l = vjs.capitalize(f[h]),
            e = window.videojs[l];
        if (e.isSupported()) {
            for (var d = 0, c = g; d < c.length; d++) {
                var k = c[d];
                if (e.canPlaySource(k)) {
                    return {
                        source: k,
                        tech: l
                    }
...
vjs.Player.prototype.src = function(b) {
    if (b instanceof Array) {
        ...
        if (b instanceof Object) {
            if (window.videojs[this.techName]["canPlaySource"](b)) {
                this.src(b.src)
            } else {
                this.src([b])
            }
    ...
    if (a !== undefined) {
        ...
        if (a !== this.userActive_) {
            ...
            if (a) {
                ...
                this.tech.one("mousemove", function(b) {
                    b.stopPropagation();
                    b.preventDefault()
                });
...
vjs.Player.prototype.listenForUserActivity = function() {
    var c, e, f, b, a, d;
    c = this.reportUserActivity;
    e = function() {
        c();
        clearInterval(f);
        f = setInterval(vjs.bind(this, c), 250)
    };
    b = function(g) {
        c();
        clearInterval(f)
    ...
    a = setInterval(vjs.bind(this, function() {
        if (this.userActivity_) {
            this.userActivity_ = false;
            this.userActive(true);
            clearTimeout(d);
            d = setTimeout(vjs.bind(this, function() {
                if (!this.userActivity_) {
...
(function() {
    ...
    b = {};
    if (c.cancelFullscreen !== undefined) {
        b.requestFn = "requestFullscreen";
        ...
        if (c[a + "RequestFullScreen"]) {
            b.requestFn = a + "RequestFullScreen";
            b.cancelFn = a + "CancelFullScreen"
        }
        b.eventName = a + "fullscreenchange"
    }
    if (document[b.cancelFn]) {
...
vjs.CurrentTimeDisplay.prototype.createEl = function() {
    ...
    a.appendChild(vjs.createEl("div").appendChild(this.content));
    return a
};
vjs.CurrentTimeDisplay.prototype.updateContent = function() {
    var a = (this.player_.scrubbing) ? this.player_.getCache().currentTime : this.player_.currentTime();
    this.content.innerHTML = '<span class="vjs-control-text">Current Time </span>' + vjs.formatTime(a, this.player_.duration())
};
...
vjs.DurationDisplay.prototype.updateContent = function() {
    ...
    if (a) {
        this.content.innerHTML = '<span class="vjs-control-text">Duration Time </span>' + vjs.formatTime(a)
    }
};
...
vjs.SeekBar.prototype.getPercent = function() {
    ...
    if (this.player_.techName === "Flash" && this.player_.seeking()) {
        var a = this.player_.getCache();
        if (a.lastSetCurrentTime) {
            b = a.lastSetCurrentTime
        } else {
    ...
    return b / this.player_.duration()
};
...
vjs.SeekBar.prototype.onMouseMove = function(b) {
    var a = this.calculateDistance(b) * this.player_.duration();
...
vjs.VolumeControl = vjs.Component.extend({
    init: function(b, a) {
        vjs.Component.call(this, b, a);
        if (b.tech && b.tech.features && b.tech.features.volumeControl === false) {
            this.addClass("vjs-hidden")
        }
        b.on("loadstart", vjs.bind(this, function() {
            if (b.tech.features && b.tech.features.volumeControl === false) {
                this.addClass("vjs-hidden")
            } else {
...
vjs.MuteToggle.prototype.update = function() {
    ...
    for (var a = 0; a < 4; a++) {
        vjs.removeClass(this.el_, "vjs-vol-" + a)
    }
    vjs.addClass(this.el_, "vjs-vol-" + c)
};
vjs.VolumeMenuButton = vjs.MenuButton.extend({
    init: function(b, a) {
        vjs.MenuButton.call(this, b, a);
        b.on("volumechange", vjs.bind(this, this.update));
        if (b.tech && b.tech.features && b.tech.features.volumeControl === false) {
            this.addClass("vjs-hidden")
        }
...
vjs.VolumeMenuButton.prototype.createMenu = function() {
    ...
    var a = new vjs.VolumeBar(this.player_, vjs.obj.merge({
        vertical: true
...
vjs.PosterImage = vjs.Button.extend({
    init: function(b, a) {
        vjs.Button.call(this, b, a);
        if (!b.poster() || !b.controls()) {
            this.hide()
        }
        b.on("play", vjs.bind(this, this.show))
    }
});
vjs.PosterImage.prototype.createEl = function() {
    ...
    if (b) {
        if ("backgroundSize" in a.style) {
            a.style.backgroundImage = 'url("' + b + '")'
        } else {
            a.appendChild(vjs.createEl("img", {
                src: b
            }))
        }
    }
...
vjs.MediaTechController.prototype.initControlsListeners = function() {
    ...
    c = this.player();
    var a = function() {
        if (c.controls() && !c.usingNativeControls()) {
            b.addControlsListeners()
        }
    };
...
vjs.Html5 = vjs.MediaTechController.extend({
    init: function(b, a, c) {
        ...
        var d = a.source;
        if (d && this.el_.currentSrc == d.src) {
            b.trigger("loadstart")
        } else {
            if (d) {
                this.el_.src = d.src
        ...
        if (vjs.TOUCH_ENABLED && b.options()["nativeControlsForTouch"] !== false) {
...
vjs.Html5.prototype.createEl = function() {
    var d = this.player_,
        e = d.tag,
        f;
    if (!e || this.features.movingMediaElementInDOM === false) {
        if (e) {
            e.player = null;
            d.tag = null;
            d.el().removeChild(e);
...
vjs.Html5.prototype.setupTriggers = function() {
    for (var a = vjs.Html5.Events.length - 1; a >= 0; a--) {
        vjs.on(this.el_, vjs.Html5.Events[a], vjs.bind(this.player_, this.eventHandler))
    }
};
...
vjs.Html5.prototype.setCurrentTime = function(b) {
    try {
        this.el_.currentTime = b
    } catch (a) {
        vjs.log(a, "Video is not ready. (Video.js)")
    }
};
...
vjs.Html5.canPlaySource = function(a) {
    try {
        return !!vjs.TEST_VID.canPlayType(a.type)
    } catch (b) {
        return ""
    }
};
...
vjs.Flash = vjs.MediaTechController.extend({
    init: function(m, n, k) {
        vjs.MediaTechController.call(this, m, n, k);
        var a = n.source,
            h = n.parentEl,
            l = this.el_ = vjs.createEl("div", {
                id: m.id() + "_temp_flash"
            }),
            b = m.id() + "_flash_api",
            c = m.options_,
        ...
        if (a) {
            if (a.type && vjs.Flash.isStreamingType(a.type)) {
                var e = vjs.Flash.streamToParts(a.src);
                j.rtmpConnection = encodeURIComponent(e.connection);
                j.rtmpStream = encodeURIComponent(e.stream)
            } else {
                j.src = encodeURIComponent(vjs.getAbsoluteURL(a.src))
            }
        }
        vjs.insertFirst(l, h);
        if (n.startTime) {
        ...
        if (n.iFrameMode === true && !vjs.IS_FIREFOX) {
            var g = vjs.createEl("iframe", {
                id: b + "_iframe",
                name: b + "_iframe",
                className: "vjs-tech",
                scrolling: "no",
            ...
            vjs.on(g, "load", vjs.bind(this, function() {
                var p, o = g.contentWindow;
                p = g.contentDocument ? g.contentDocument : g.contentWindow.document;
                p.write(vjs.Flash.getEmbedCode(n.swf, j, d, f));
                o.player = this.player_;
                o.ready = vjs.bind(this.player_, function(r) {
                    var t = p.getElementById(r),
                        s = this,
                        q = s.tech;
                    q.el_ = t;
                    vjs.Flash.checkReady(q)
                });
                o.events = vjs.bind(this.player_, function(r, q) {
                    var s = this;
                    if (s && s.techName === "flash") {
                        s.trigger(q)
                    }
                });
                o.errors = vjs.bind(this.player_, function(r, q) {
                    vjs.log("Flash Error", q)
                })
            }));
            l.parentNode.replaceChild(g, l)
        } else {
            vjs.Flash.embed(n.swf, l, j, d, f)
        }
    }
});
...
vjs.Flash.prototype.src = function(b) {
    if (vjs.Flash.isStreamingSrc(b)) {
        b = vjs.Flash.streamToParts(b);
        this.setRtmpConnection(b.connection);
        this.setRtmpStream(b.stream)
    } else {
        b = vjs.getAbsoluteURL(b);
        this.el_.vjs_src(b)
    }
...
vjs.Flash.prototype.currentSrc = function() {
    var c = this.el_.vjs_getProperty("currentSrc");
    if (c == null) {
        var a = this.rtmpConnection(),
            b = this.rtmpStream();
        if (a && b) {
            c = vjs.Flash.streamFromParts(a, b)
        }
    }
    return c
};
...
var createSetter = function(a) {
    var b = a.charAt(0).toUpperCase() + a.slice(1);
    api["set" + b] = function(c) {
        return this.el_.vjs_setProperty(a, c)
    }
};
var createGetter = function(a) {
    api[a] = function() {
...
(function() {
    var a;
    for (a = 0; a < readWrite.length; a++) {
        createGetter(readWrite[a]);
        createSetter(readWrite[a])
    }
    for (a = 0; a < readOnly.length; a++) {
        createGetter(readOnly[a])
    }
})();
...
vjs.Flash.canPlaySource = function(a) {
    if (a.type in vjs.Flash.formats || a.type in vjs.Flash.streamingFormats) {
        return "maybe"
    }
};
vjs.Flash.formats = {
...
vjs.Flash.onReady = function(b) {
    var d = vjs.el(b);
    var c = d.player || d.parentNode.player,
        a = c.tech;
    d.player = c;
    a.el_ = d;
...
vjs.Flash.onEvent = function(b, a) {
    var c = vjs.el(b)["player"];
    c.trigger(a)
};
vjs.Flash.onError = function(a, c) {
    var b = vjs.el(a)["player"];
    b.trigger("error");
    ...
    try {
        ...
        try {
            if (navigator.mimeTypes["application/x-shockwave-flash"].enabledPlugin) {
                a = (navigator.plugins["Shockwave Flash 2.0"] || navigator.plugins["Shockwave Flash"]).description.replace(/\D+/g, ",").match(/^,?(.+),?$/)[1]
            }
        } catch (b) {}
    }
    return a.split(",")
};
vjs.Flash.embed = function(b, j, g, c, e) {
...
vjs.Flash.getEmbedCode = function(e, h, g, b) {
    var c = '<object type="application/x-shockwave-flash"',
        a = "",
        f = "",
        d = "";
    if (h) {
        vjs.obj.each(h, function(j, k) {
            a += (j + "=" + k + "&amp;")
        })
    }
    g = vjs.obj.merge({
    ...
    vjs.obj.each(g, function(j, k) {
        f += '<param name="' + j + '" value="' + k + '" />'
    });
    b = vjs.obj.merge({
        data: e,
        width: "100%",
...
vjs.Flash.streamToParts = function(d) {
    var c = {
        connection: "",
        stream: ""
    };
...
vjs.MediaLoader = vjs.Component.extend({
    init: function(e, c, f) {
        vjs.Component.call(this, e, c, f);
        if (!e.options_.sources || e.options_.sources.length === 0) {
            for (var d = 0, b = e.options_.techOrder; d < b.length; d++) {
                var g = vjs.capitalize(b[d]),
                    a = window.videojs[g];
                if (a && a.isSupported()) {
                    e.loadTech(g);
                    break
...
vjs.Player.prototype.addTextTrack = function(f, e, g, d) {
    var c = this.textTracks_ = this.textTracks_ || [];
    d = d || {};
    d.kind = f;
    d.label = e;
    d.language = g;
...
vjs.Player.prototype.addTextTracks = function(b) {
    var c;
...
vjs.Player.prototype.showTextTrack = function(h, f) {
    var c = this.textTracks_,
        d = 0,
        b = c.length,
        a, g, e;
    for (; d < b; d++) {
...
vjs.TextTrack.prototype.parseCues = function(b) {
    var f, c, g, h = b.split("\n"),
        k = "",
        a;
    for (var e = 1, d = h.length; e < d; e++) {
        k = vjs.trim(h[e]);
        if (k) {
            if (k.indexOf("-->") == -1) {
                a = k;
...
vjs.TextTrack.prototype.parseCueTime = function(b) {
    var g = b.split(":"),
        f = 0,
        c, e, a, h, d;
    if (g.length == 3) {
...
vjs.TextTrack.prototype.update = function() {
    if (this.cues_.length > 0) {
        ...
        if (this.prevChange === undefined || b < this.prevChange || this.nextChange <= b) {
            var f = this.cues_,
                k = this.player_.duration(),
                j = 0,
                g = false,
                a = [],
                h, e, d, c;
            if (b >= this.nextChange || this.nextChange === undefined) {
                c = (this.firstActiveIndex !== undefined) ? this.firstActiveIndex : 0
            } else {
                g = true;
            ...
            while (true) {
                d = f[c];
                if (d.endTime <= b) {
                    j = Math.max(j, d.endTime);
                    if (d.active) {
                        d.active = false
                    }
                } else {
                    if (b < d.startTime) {
                        k = Math.min(k, d.startTime);
                        if (d.active) {
                            d.active = false
                        }
                        if (!g) {
...
vjs.TextTrack.prototype.updateDisplay = function() {
    var b = this.activeCues_,
        d = "",
        c = 0,
        a = b.length;
    for (; c < a; c++) {
        d += '<span class="vjs-tt-cue">' + b[c].text + "</span>"
    }
...
vjs.TextTrackDisplay = vjs.Component.extend({
    init: function(b, a, c) {
        vjs.Component.call(this, b, a, c);
        if (b.options_.tracks && b.options_.tracks.length > 0) {
            this.player_.addTextTracks(b.options_.tracks)
        }
    }
});
...
vjs.OffTextTrackMenuItem.prototype.update = function() {
    ...
    for (; d < b; d++) {
        a = c[d];
        if (a.kind() == this.track.kind() && a.mode() == 2) {
            e = false
        }
    }
    this.selected(e)
...
vjs.TextTrackButton.prototype.createItems = function() {
    ...
    for (var c = 0; c < this.player_.textTracks().length; c++) {
        a = this.player_.textTracks()[c];
        if (a.kind() === this.kind_) {
            b.push(new vjs.TextTrackMenuItem(this.player_, {
                track: a
            }))
        }
    }
...
vjs.CaptionsButton = vjs.TextTrackButton.extend({
    init: function(b, a, c) {
        vjs.TextTrackButton.call(this, b, a, c);
...
vjs.ChaptersButton.prototype.createMenu = function() {
    ...
    for (; e < c; e++) {
        b = h[e];
        if (b.kind() == this.kind_ && b.dflt()) {
            if (b.readyState() < 2) {
                this.chaptersTrack = b;
    ...
    a.el_.appendChild(vjs.createEl("li", {
        className: "vjs-menu-title",
        innerHTML: vjs.capitalize(this.kind_),
        tabindex: -1
    }));
    if (l) {
        ...
        for (; e < c; e++) {
            d = f[e];
            g = new vjs.ChaptersTrackMenuItem(this.player_, {
                track: l,
                cue: d
            });
            k.push(g);
...
vjs.autoSetup = function() {
    var d, a, f, c = document.getElementsByTagName("video");
    if (c && c.length > 0) {
        for (var e = 0, b = c.length; e < b; e++) {
            a = c[e];
            if (a && a.getAttribute) {
                if (a.player === undefined) {
                    d = a.getAttribute("data-setup");
                    if (d !== null) {
                        d = vjs.JSON.parse(d || "{}");
                        f = videojs(a, d)
                    }
